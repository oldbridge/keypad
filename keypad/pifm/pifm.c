...
void playWav(char* filename, float samplerate,float bw)
{
    int fp= STDIN_FILENO;
    ...
    while (read(fp, &data, 2)) {
        ...
        float dval = sample*bw; // actual transmitted sample.  bw is bandwidth (1 is about 12 kHz and 15 is about 75 kHz)
...
int main(int argc, char **argv)
{
    ...
    if (argc>1) {
      ...
      playWav(argv[1], argc>3?atof(argv[3]):22500,argc>4?atof(argv[4]):15.0);
    } else
